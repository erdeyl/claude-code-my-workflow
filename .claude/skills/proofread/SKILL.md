---
name: proofread
description: Runs the proofreading protocol on lecture files. Checks grammar, typos, overflow, consistency, and academic writing quality. Produces a report without editing files.
...
1. **Identify files to review:**
   ...
   - If `$ARGUMENTS` is "all": review all lecture files (auto-detect `Slides/`, `Quarto/`, `decks/`)
...
4. **Save each report** to auto-detected output directory (`quality_reports/` or current directory):
   - For `.tex` files: `FILENAME_report.md`
   - For `.qmd` files: `FILENAME_qmd_report.md`
